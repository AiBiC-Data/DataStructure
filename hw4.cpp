...
// structure : weighted edge in graph
struct Edge{
    int src, dest, weight;
};

// structure to represent : connected, undirected and weighted graph
...
//structure : subset for union-find
struct subset{
    int parent;
    int rank;
};
...
int main() {
    ...
    graph->edge[10].src = d;
    graph->edge[10].dest = e;
    graph->edge[10].weight = 4;

    graph->edge[11].src = d;
    graph->edge[11].dest = f;
